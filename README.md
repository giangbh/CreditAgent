...
10. **[Architecture Flow Diagram (Interactive HTML)](docs/architecture_diagram.html):** Sơ đồ tương tác toàn bộ 6 tầng kiến trúc hệ thống CreditAgent (Interface, Temporal Engine, 13 Agents Pipeline, Deterministic Control, Human Authority, Persistence/Backends).
11. **[Temporal.io Workflow Diagram (Interactive HTML)](docs/temporal_workflow_diagram.html):** Sơ đồ quy trình 6 bước thực thi luồng Temporal.io Durable Execution (Client, Server, Task Queue `credit-approval-queue`, Worker, `@workflow.defn` & `@activity.defn`).
12. Xem thêm [kiến trúc Multi-Agent đồng phê duyệt tín dụng](kien-truc-multi-agent-dong-phe-duyet-tin-dung.md) gốc.
